...
std::function<void()> eventHandler = [&](){
  ...
  if(!Tiny::event.keys.empty()){
    if(Tiny::event.keys.back().key.keysym.sym == SDLK_w) py -= 0.1*zoom;
    else if(Tiny::event.keys.back().key.keysym.sym == SDLK_a) px += 0.1*zoom;
    else if(Tiny::event.keys.back().key.keysym.sym == SDLK_s) py += 0.1*zoom;
    else if(Tiny::event.keys.back().key.keysym.sym == SDLK_d) px -= 0.1*zoom;
  }
